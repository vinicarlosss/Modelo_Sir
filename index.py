...
for i in range(7, 60):
    if i == 59:
        ...
        df = df.rename(columns={f'Unnamed: {i}': f'Semana_{semana}'})
        semana += 1
...
for i in range(1, 53):
    semanas_infectados[f'Semana_{i}'] = linha_sp[f'Semana_{i}']
...
#plt.plot(semanas_infectados.columns.tolist(), valores_sucetiveis_normalizados, label="Sucetíveis", color='green', marker='.')
#plt.plot(semanas_infectados.columns.tolist(), valores_infectados_normalizados, label="Infectados", color='red', marker='.')
